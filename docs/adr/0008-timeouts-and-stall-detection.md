...
### 2. Progress signal: Gitea job-log growth, read via the Gitea API, not pod-phase or kubectl logs

Two v1 designs were tried and invalidated by live testing against a real cluster before
landing on this one:

- **Pod-phase transitions only.** A job that hangs mid-step produces no pod-level
  signal at all, so a hung job and a slow-but-progressing job both look identical
  (`Running`, no phase change) for the entire stall window -- exactly the false-negative
  case this ADR exists to avoid narrowing, not widening.
- **Tailing the runner container's stdout (`kubectl logs`).** Verified live against a
  test job that ticked output every 5 seconds: `act_runner`'s host backend does **not**
  stream job step output to the container's own stdout at all -- only its own daemon
  messages ("Registering runner", "Starting runner daemon"). This caused a genuinely
  slow-but-alive job to be wrongly killed at 21s against a 20s stall window in testing.
  Not a bug in the log-tail code; a wrong premise about where act_runner writes
  progress.

The verified-working signal is `act_runner` streaming step output to **Gitea** via its
own `UpdateLog`/gRPC protocol, independent of container stdout. Gitea exposes this as a
job-log HTTP endpoint (`GET /repos/{owner}/{repo}/actions/jobs/{job_id}/logs`, added in
Gitea PR #33858) which was confirmed live to show real-time step output as a job ran.
v1's progress signal is therefore:
...
2. **Liveness check**: `GET {job.url}/logs`, reading only the `Content-Length` header
   ...
   `lastJobLogSize`; this is a liveness probe only -- log content is never parsed,
...
content and job/task status semantics.

`Gitea task API's updated_at` field was also considered and ruled out: verified live
...
2. **Whether the per-reconcile Gitea API call (job discovery + log-size check) needs
   caching or a longer poll interval** if the org has a large number of concurrently
   in-progress jobs across many repos, to bound Gitea API load. Not yet a problem at
   current fleet scale; revisit if `ListOrgInProgressJobs` response size or latency
   becomes a concern.
